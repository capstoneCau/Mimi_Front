...
export const createRoomAsync = (init_users, available_dates, user_limit, introduction, token) => async (dispach, getState) => {
...
export const getOwnsRoomList = (user = null, token) => async (dispach, getState) => {
  const res = await fetch(SERVER_DOMAIN + `meeting/ownsRoomList/` + user != null ? `${room_id}/` : '', {
    method:"GET",
    mode:'cors',
    headers : {
      'Authorization' : `Token ${token}`,
